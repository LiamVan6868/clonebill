<?php
// Main Router (adapted for Railway/Caddy)

$uri = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);

// Handle Root Request - Redirect to /trang-chu to match original behavior
...
foreach ($rules as $pattern => $target) {
    if (preg_match('#' . $pattern . '#', ltrim($uri, '/'), $matches)) {
        $targetUrl = preg_replace('#' . $pattern . '#', $target, ltrim($uri, '/'));
        ...
        if (file_exists($fileToInclude)) {
            ...
            echo "Router (index.php): Target file not found: " . $parsed['path'];
            exit;
        }
    }
}
